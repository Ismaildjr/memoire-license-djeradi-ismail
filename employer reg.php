...
if (isset($_POST['submit'])) {
    ...
    if (!preg_match('/^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$/', $email)) {
        ...
        if (mysqli_num_rows($select) > 0) {
        ...
        else if(empty($name) || empty($email) || empty($num) || empty($pass) || empty($skill) || empty($exp) || empty($exper) || empty($wilayas) || empty($communes) || empty($price)) {
            ...
            $insert = mysqli_query($conn, "INSERT INTO `users`(full_name, email, number, password,skills,experience,exper,wilaya,communes,price,pp) VALUES('$name', '$email','$num', '$pass','$skill','$exp','$exper','$wilayas','$communes','$price','$new_img_name')")
...
$sql = "SELECT id  , nom FROM `wilayas`";
  ...
  <div class="form-group"> 
    <ul class="list-group">
        <li>
    <label class="control-label col-sm-4" for="communes">Communes</label>
        <div class="col-sm-4">
             <select name="communes" id="communes" required>
