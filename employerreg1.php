...
if (isset($_POST['submit'])) {
    ...
    if (!preg_match('/^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$/', $email)) {
        ...
        $select11 = mysqli_query($conn, "SELECT * FROM `users2` WHERE email='$email'") or die('error');
        $select111 = mysqli_query($conn, "SELECT * FROM `users2` WHERE number='$num'") or die('error');
      ...
      //  else if(empty($name) || empty($email) || empty($num) || empty($pass) ||  empty($wilayas) || empty($communes)|| empty($exp) || empty($exper) || empty($skill)|| empty($price)) {
       ...
       // }
            ...
            $insert = mysqli_query($conn, "INSERT INTO `users`(full_name, email,password, number, wilaya,communes,pp,verify_token,salt,skills,experience,exper,price) VALUES('$name', '$email', '$hashedPassword','$num','$wilayas','$communes','$new_img_name','$verify_token','$salt','$skill','$exp','$exper','$price')")
