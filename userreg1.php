...
function sendemail_verify($name,$email,$verify_token){
 ...
 <body>
     ...
     <p>Thank you for registering with <span style=color: #0071c5;' class='white'>ManoPro</span>. Please click the button below to verify your email address and activate your account:</p>
     <a href='http://localhost/cr1/verify-email.php?token=$verify_token' class='btn'><span class='white'>Verify Email Address</span></a>
     <p>If you did not create an account with <span class='white' style=color: #0071c5;'>ManoPro</span>, please ignore this email.</p>
 </div>
...
if (isset($_POST['submit'])) {
    ...
    $salt = random_bytes(16); // Generate a 16-byte salt
    ...
    if (!preg_match('/^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$/', $email)) {
        ...
        if (mysqli_num_rows($select) > 0) {
            ...
            $insert = mysqli_query($conn, "INSERT INTO `users2`(full_name, email,password, number, wilaya,communes,pp,verify_token,salt) VALUES('$name', '$email', '$hashedPassword','$num','$wilayas','$communes','$new_img_name','$verify_token','$salt')")
